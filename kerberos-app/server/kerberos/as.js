...
class AuthenticationServer {
  ...
  authenticate(username, password) {
    ...
    if (!validUsers[username] || validUsers[username] !== password) throw new Error('Invalid credentials');
    ...
    console.log(`AS-server: TGT for ${username}`);
  ...
  validateTGT(tgtId) {
    ...
    const hoursDiff = (currentTime - issuedTime) / (60 * 1000);

    if (hoursDiff > 60) return { valid: false, reason: 'TGT expired' };
