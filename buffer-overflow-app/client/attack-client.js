...
class AttackClient {
    ...
    // Generate buffer overflow attack payload
    ...
    // Attempt buffer overflow attack
...
async function runAttackClient() {
    const attacker = new AttackClient('http://localhost:3000'); // Target vulnerable server
    
    console.log('TARGET: Vulnerable Server on http://localhost:3000');
    await attacker.runAttackSuite();
    
    // Try to bypass protection
    const protectedAttacker = new AttackClient('http://localhost:3001'); // Target protected server
